class BaseRemover {
    ...
    get formatErrorMessage() {
        return 'Неверный формат удаления';
    }
    ...
    async executeRemover (inData) {
        throw new Error('executeRemover must be implemented');
    }
    ...
    async execute (inData) {
        ...
        let response;
        try {
            const result = await this.executeRemover(Object.assign(inData.body, { transaction: inData.transaction }));
