...
class ApproveOrderRent {
    ...
    static get url () {
        return '/proxy/approve-order-rent.json';
    }
    ...
    async approveOrderRent (inData) {
        const defaults = {
            status: 'active',
            finishDate: new Date(Date.now() + 20 * 24 * 60 * 60 * 1000)
        };
    ...
    async executeApproveOrderRent (inData) {
        await this.validate(inData);
        const id = await this.approveOrderRent(inData);
        return { id: id };
    }
    ...
    async execute (inData) {
        ...
        try {
            ...
            const result = await this.executeApproveOrderRent(Object.assign(inData.body, { transaction: inData.transaction }));
...
module.exports = ApproveOrderRent;